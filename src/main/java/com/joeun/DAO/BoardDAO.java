...
public class BoardDAO extends JDBConnection {
	...
	public List<Board> list() {
		...
		try {
			// 쿼리 실행(SQL) 객체 생성 - Statement stmt
			stmt = con.createStatement();
			rs = stmt.executeQuery(sql);
			
			//조회 결과를 리스트(boardList)에 추가
			while ( rs.next() ) { // next(): 실행결과의 다음 데이터로 이동
	...
	public Board select(int no) {
		...
		try {
			// 쿼리 실행(SQL) 객체 생성 - PreparedStatement (psmt)
	...
	public int insert(Board board) {
		...
		try {
			...
			// executeUpdate();
			// :SQL (INSERT, UPDATE, DELETE)을 실행하고,
			//  적용된 데이터 갯수를 INT 타입으로 반환.
	...
	public int update(Board board) {
		...
		try {
			...
			psmt.setString(2, board.getWriter());	// 2번 ?에 작성자 매핑
			psmt.setString(3, board.getContent());	// 3번 ?에 내용을 매핑
			psmt.setInt(4, board.getNo());			// 4번 ?에 게시글 번호을 매핑
